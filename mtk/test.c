...
#define WMT_FM_COMM     2
#define WMT_FM_I2C      1


int main(int argc, char **argv)
{
    int fd, fd_stp, fd_hif;
    int arg;
    unsigned int i;
    char buf[12];

    fd_stp = open("/dev/stpwmt", O_RDWR | O_NOCTTY);
    if(fd_stp < 0)
    {
        printf("open stpwmt failed\r\n");
        return -1;
    }
    printf("open stpwmt success, argv[1] = %c\r\n", *argv[1]);

    sleep(2);

    ioctl(fd_stp, WMT_IOCTL_SET_STP_MODE, 1);

    fd = open("/dev/wmtWifi", O_RDWR);
    if(fd < 0)
    {
        printf("open wmtwifi failed\r\n");
        return -1;
    }
    printf("open wmtwifi success, argv[1] = %c\r\n", *argv[1]);
