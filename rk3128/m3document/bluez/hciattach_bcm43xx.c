		...
		const char *bdaddr)
{
	...
	if (bcm43xx_locate_patch(FIRMWARE_DIR, chip_name, fw_path)) {
        ...
        sleep(1);
        printf("sleep 1 sec end\r\n");

        if (bcm43xx_reset(fd))
			return -1;
	}
    
    printf("6212 reset OK!\r\n");

	if (bdaddr)
		bcm43xx_set_bdaddr(fd, bdaddr);

    printf("6212 set bdaddr OK!\r\n");
	if (bcm43xx_set_speed(fd, ti, speed))
		return -1;

    printf("6212 set speed OK!\r\n");
	return 0;
}
