...
#define CONFIG_RK818_SCREEN_ON_VOL_THRESD	0
#define CONFIG_RK818_SYSTEM_ON_VOL_THRESD	3700
...
//		return ((battery.voltage_uV < CONFIG_RK818_SYSTEM_ON_VOL_THRESD) ||(battery.capacity<CONFIG_RK818_SYSTEM_ON_CAPACITY_THRESD))? 1:0;
	...
	if(battery.voltage_uV >  CONFIG_RK818_SYSTEM_ON_VOL_THRESD && battery.capacity > CONFIG_RK818_SYSTEM_ON_CAPACITY_THRESD)

		return 0;
	else
 		return 1;//(battery.voltage_uV < CONFIG_SYSTEM_ON_VOL_THRESD) ? 1:0;
#endif
}
...
#endif
#if defined(CONFIG_POWER_RK818)
	printf("CONFIG_POWER_RK818\n");
	if (fg_rk818_init(bus,0x1c) >= 0) {
		printf("CONFIG_POWER_RK818 ok \n");
	}
