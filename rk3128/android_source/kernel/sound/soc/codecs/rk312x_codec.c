...
int switch_to_spk(int enable)
{
	unsigned int val = 0;
	if(hdmi_state == 1 && enable == 1)
		return 0;
	if (!rk312x_priv) {
...
extern void hdmi_audio_enable(int mute);
...
extern int vcc_codec_vcc_enable(int value);
...
static int rk312x_platform_probe(struct platform_device *pdev)
{
	...
	if(rk312x->spk_hp_switch_gpio != INVALID_GPIO) {
		...
		gpio_direction_output(rk312x->spk_hp_switch_gpio, !rk312x->spk_io);
	}
#endif
