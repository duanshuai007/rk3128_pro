			       ...
			       char *buf)
{
	...
	dev_err(es705->dev, "%s(): power state = %s\n", __func__,
...
{
	struct escore_priv *es705 = &escore_priv;
	dev_err(es705->dev, "%s(): GPIO reset\n", __func__);
	es705->mode = SBL;
	escore_gpio_reset(es705);
	dev_err(es705->dev, "%s(): Ready for STANDARD download by proxy\n",
		__func__);
	return count;
}
					...
					const char *buf, size_t count)
{
	...
	dev_err(es705->dev, "%s(): Set Overlay mode\n", __func__);
	...
	if (rc) {
		...
		dev_err(es705->dev, "%s(): Ready for VOICESENSE download by proxy\n",
		__func__);
				...
				const char *buf, size_t count)
{
	...
	dev_err(es705->dev, "%s(): Enable Voice Sense Event to Host\n",
		__func__);
...
{
	dev_err(es705->dev, "%s(): VS -> NS Switing\n", __func__);
				...
				unsigned int set_power_state_cmd)
{
	...
	dev_err(escore_priv.dev, "%s(): Power state change successful\n",
		__func__);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_err(escore_priv.dev, "%s(): Current state:%s, Requested state:%s\n",
		__func__,
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_err(escore_priv.dev, "%s(): rx1_route_enable = %d\n",
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_err(escore_priv.dev, "%s: Response 0x%08X", __func__, rspn);
	return 0;
}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_err(escore_priv.dev, "%s(): tx1_route_enable = %d\n",
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_err(escore_priv.dev, "%s(): rx2_route_enable = %d\n",
...
static int es705_get_ns_value(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	dev_err(escore_priv.dev, "%s(): NS = %d\n",
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.enumerated.item[0];
	int rc = 0;
	dev_err(escore_priv.dev, "%s(): SW = %d\n", __func__, value);
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.enumerated.item[0];
	int rc = 0;
	dev_err(escore_priv.dev, "%s(): STS = %d\n", __func__, value);
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.enumerated.item[0];
	int rc = 0;
	dev_err(escore_priv.dev, "%s(): RX_NS = %d\n", __func__, value);
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.enumerated.item[0];
	int rc = 0;
	dev_err(escore_priv.dev, "%s(): WNF = %d\n", __func__, value);
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.enumerated.item[0];
	int rc = 0;
	dev_err(escore_priv.dev, "%s(): BWE = %d\n", __func__, value);
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.enumerated.item[0];
	int rc = 0;
	dev_err(escore_priv.dev, "%s(): Avalon Wind Noise = %d\n",
		__func__, value);
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.enumerated.item[0];
	int rc = 0;
	dev_err(escore_priv.dev, "%s(): Virtual Bass Boost = %d\n",
		__func__, value);
...
static int es705_get_aud_focus(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	dev_err(escore_priv.dev, "%s(): Focus = %d\n",
...
{
	dev_err(escore_priv.dev, "%s():dai->name = %s dai->id = %d\n",
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	int rc = 0;
	dev_err(es705->dev, "%s:internal_rate  %d ucontrol %d ",
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_err(es705->dev, "%s():es705->internal_rate = %d ucontrol = %d\n",
		__func__, (int)es705->internal_rate,
		(int)ucontrol->value.enumerated.item[0]);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	for (count = 0; count < ES705_PRESET_ARRAY_SIZE; count++) {
		...
		dev_err(escore_priv.dev, "%s(): Set Preset value:%d\n",
				      ...
				      unsigned int *rx_slot)
{
	...
	dev_err(codec->dev, "%s(): dai->name = %s, dai->id = %d\n", __func__,
		dai->name, dai->id);
...
int es705_remote_add_codec_controls(struct snd_soc_codec *codec)
{
	...
	printk("es705_remote_add_codec_controls hyman \n");
...
irqreturn_t es705_cmd_completion_isr(int irq, void *data)
{
	...
	printk("%s(): API Interrupt received\n", __func__);
	/* Complete if expected */
	if (escore->wait_api_intr) {
		printk("%s(): API Interrupt completion.\n",
				__func__);
...
repeat:
	...
	if (event_type != ES_NO_EVENT) {
		printk("%s(): Notify subscribers about 0x%04x event\n",
				__func__, event_type);
...
/* API Interrupt registration */
	...
	dev_err(dev, "%s(): gpioa configured\n", __func__);
	...
	if (of_property_read_u32(dev->of_node, "adnc,gpio_b_irq_type", &val)) {
		dev_err(dev, "%s(): Default gpio_b_irq_type = %d\n",
					__func__, ES_RISING_EDGE);
		pdata->gpio_b_irq_type = ES_RISING_EDGE;
	} else {
		dev_err(dev, "%s(): gpio_b_irq_type = %d\n",
					__func__, val);
...
int es705_core_probe(struct device *dev)
{
	...
	const char *fw_filename = "audience/es814/BoskoUnzipApp.bin";222222
#else
	const char *fw_filename = "audience/es814/audience-es814-fw_4m.bin"; asdasdf
#endif
#else
...
#else
	const char *fw_filename = "audience/es705/audience-es705-fw.bin";333333
...
*/
...
#ifdef CONFIG_SND_SOC_ES705_EXTCLK_OVER_GPIO
	dev_err(es705_priv.dev, "%s(): extclk_gpio = %d\n",
		__func__, pdata->extclk_gpio);
	...
	dev_err(escore_priv.dev, "%s(): wakeup_gpio = %d\n", __func__,
		pdata->wakeup_gpio);
	...
	dev_err(dev, "%s(): gpioa not configured\n", __func__);
	pdata->gpioa_gpio = -1;
#endif
	dev_err(escore_priv.dev, "%s(): gpioa_gpio = %d\n", __func__,
		pdata->gpioa_gpio);
	...
	dev_err(escore_priv.dev, "%s(): gpiob_gpio = %d\n", __func__,
		pdata->gpiob_gpio);
	...
	dev_err(escore_priv.dev, "%s(): reset_gpio = %d\n", __func__,
		pdata->reset_gpio);
...
pdata_error:
	dev_err(escore_priv.dev, "%s(): exit with error\n", __func__);
	return rc;
}
#ifdef CONFIG_ARCH_MSM
