...
static int escore_runtime_suspend(struct device *dev)
{
	...
	if (!es_ready_to_suspend(escore)) {
		printk("%s() - Not ready for suspend\n", __func__);
		return -EBUSY;
	}
...
out:
	...
	if (ret < 0) {
		printk( "%s() Firmware recovery failed %d\n",
							__func__, ret);
	}
...
static int escore_generic_suspend(struct device *dev)
{
	...
	printk("escore_generic_suspend %s()\n", __func__);
	...
	printk("escore_generic_suspend %s() complete %d\n", __func__, ret);
...
static int escore_generic_resume(struct device *dev)
{
	...
	printk("escore_generic_resume %s()\n", __func__);
	...
	printk("escore_generic_resume %s() complete %d\n", __func__, ret);
