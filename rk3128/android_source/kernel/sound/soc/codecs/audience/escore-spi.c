...
static int escore_spi_boot_setup(struct escore_priv *escore)
{
	...
	printk("%s(): prepare for fw download\n", __func__);
	...
	usleep_range(1000, 1050);
	printk("%s(): write ES_BOOT_CMD = 0x%04x\n", __func__, boot_cmd);
...
static int escore_spi_populate_dt_data(struct escore_priv *escore)
{
	...
	if (rc || pdata->spi_wdb_speed == 0) {
		...
		printk("%s, Use operational speed for writing data block\n",
				__func__);
