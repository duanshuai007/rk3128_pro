...
int escore_cmd_nopm(struct escore_priv *escore, u32 cmd, u32 *resp)
{
	...
	*resp = 0;
	...
	if (err || sr)
		goto exit;
	printk("escore_cmd_nopm no response to command 0x%08x\n", cmd);
	if ((*resp) == 0) {
		err = -ETIMEDOUT;
...
void escore_load_std_fw(const struct firmware *fw, void *context)
{
	...
	dev_err(escore->dev, "%s(): NS asynchronous firmware request\n",
			__func__);
	if ((escore == NULL) )
			dev_err(escore->dev, "%s(): escore null \n",
			__func__);
	if ((fw == NULL))
			dev_err(escore->dev, "%s(): fw null \n",
			__func__);
		...
		size_t len)
{
	...
	defined(CONFIG_SND_SOC_ES_HIGH_BW_BUS_SPI)
			...
			if (resp == 0) {
				printk("%s(): Invalid response 0x%0x\n",
						__func__, resp);
				rc = -EIO;
				continue;
			} else if ((resp >> 16) == 0) {
			...
			if (resp != (ES_WRITE_DATA_BLOCK << 16)) {
				printk("%s(): response not ready 0x%0x\n",
						__func__, resp);
				rc = -EIO;
			} else {
				break;
...
int escore_reconfig_api_intr(struct escore_priv *escore)
{
	...
	printk("%s(): Reconfig API Interrupt with mode %d\n",
...
int escore_api_intr_wait_completion(struct escore_priv *escore)
{
	...
	printk("%s(): Waiting for API interrupt", __func__);
...
int escore_switch_ext_osc(struct escore_priv *escore)
{
	...
	dev_err(escore->dev, "%s(): Switch ext oscillator", __func__);
...
int escore_ns_to_af(struct escore_priv *escore)
{
	...
	printk("%s(): AF Firmware download successful\n", __func__);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (value) {
		...
		if (escore->mode != AF) {
			printk("%s(): AF state already disabled\n", __func__);
			goto exit;
		}

		if (atomic_read(&escore->active_streams)) {
			printk("%s(): Active stream detected\n", __func__);
			goto exit;
		}
		...
		int context)
{
	...
	if (escore->intf_probed != (escore->pri_intf | escore->high_bw_intf)) {
		printk("%s(): Both interfaces are not probed %d\n",
...
int escore_platform_init(void)
{
	...
	printk("%s(): Registered escore platform driver", __func__);
...
int escore_device_startup_seq(struct device *dev)
{
	...
	power_gpio = of_get_named_gpio(dev->of_node, "adnc,power-gpio", 0);
	if (power_gpio < 0) {
		pr_err("%s,hyman get power gpio failed\n",__func__);
		rc = -1;
		goto power_gpio_request_error;
	}
	dev_err(dev, "%s(): power gpio %d\n",__func__, power_gpio);
	...
	dev_err(dev,"%s(): escore_power gpio is enabled", __func__);
	msleep(25);
#endif	
