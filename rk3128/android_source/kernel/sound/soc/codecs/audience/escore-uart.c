...
static int set_sbl_baud_rate(struct escore_priv *escore, u32 sbl_rate_req_cmd)
{
	...
	printk("Setting uart baud rate %x\n", sbl_rate_req_cmd);
...
int escore_uart_boot_sequence(struct escore_priv *escore)
{
	...
	/* SBL SYNC BYTE 0x00 */
	pr_err("%s(): write ESCORE_SBL_SYNC_CMD = 0x%02x\n", __func__,
		sbl_sync_cmd);
	...
	pr_err("%s(): firmware load sbl sync write rc=%d\n", __func__, rc);
	memset(msg, 0, 4);
	...
	/* SBL BOOT BYTE 0x01 */
	memset(msg, 0, 4);
	pr_err("%s(): write ESCORE_SBL_BOOT_CMD = 0x%02x\n", __func__,
		sbl_boot_cmd);
...
int escore_uart_boot_finish(struct escore_priv *escore)
{
	...
	do {
		printk("%s(): write ES_SYNC_CMD = 0x%08x\n",
			__func__, sync_cmd);
...
//extern int escore_i2c_boot_finish(struct escore_priv *escore);
static int escore_uart_setup_high_bw_intf(struct escore_priv *escore)
{
	...
	escore->boot_ops.finish = escore_i2c_boot_finish;//escore_uart_boot_finish;
