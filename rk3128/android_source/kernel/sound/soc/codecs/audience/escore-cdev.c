...
{
	printk("escore: ignored preset id = %i\n", id);
	return 0;
}
...
int macro_cmd(void *ctx, u32 cmd)
{
	...
	if (!(cmd & BIT(28))) {
		printk("escore: cmd=0x%08x Response:0x%08x\n", cmd,
				escore->bus.last_response);
	} else {
		printk("escore: cmd=0x%08x\n", cmd);
	}
	return rc;
}
	...
	while ((tok = strsep(&cur, " \t\r\n")) != cur) {
		...
		case PT_ID:
			if (sscanf(tok, "%d", &id) == 1) {
				...
				printk("expecting preset id: got '%s'\n",
					 tok);
				err = -EINVAL;
				goto EXIT;
			}
		...
		case PT_HEXWORD:
			if (last_token == PT_HEXWORD &&
				...
				printk("expecting hex word: got '%s'\n",
					 tok);
				err = -EINVAL;
				goto EXIT;
			}
				...
				size_t count, loff_t *f_pos)
{
	...
	while (used < count) {
		...
		if (space == 0) {
			printk("escore: line too long - exhausted buffer\n");
			err = -EFBIG;
			goto OUT;
		}
		...
		printk("escore: copying fragment size = %i\n", frag);
		err  = copy_from_user(data, ptr, frag);
		if (err) {
			printk("escore: error copying user data\n");
			err = -EFAULT;
			goto OUT;
		}
		used += frag;
...
static int firmware_open(struct inode *inode, struct file *filp)
{
	...
	printk("firmware download setup ok\n");
...
firmware_release_exit:
	...
	printk("successful download of firmware\n");
	return ret;
}
			      ...
			      size_t count, loff_t *f_pos)
{
	...
	printk("firmware write count=%zd bytes\n", count);

	wr = bs = 0;
	while (wr < count) {
		bs = min(count - wr, FW_BUF_SIZE);
		BUG_ON(bs == 0);
		printk("wr=%zd bs=%zd buf+wr=%p\n", wr, bs, buf + wr);
	...
	struct file *filp)
{
	...
	*/
	...
	printk("%s(): streaming mutex lock killable\n", __func__);
...
streamdev_invalid_param_err:
	printk("%s(): streaming mutex unlock\n", __func__);
	...
	struct inode *inode)
{
	...
	printk("%s(): stopping stream kthread\n", __func__);
