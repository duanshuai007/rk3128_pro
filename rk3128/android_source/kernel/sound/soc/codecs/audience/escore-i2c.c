	...
	*resp = 0;
	...
	do {
		if (escore->cmd_compl_mode == ES_CMD_COMP_INTR) {
			printk("%s(): Waiting for API interrupt. Jiffies:%lu",
					__func__, jiffies);
...
{
	int rc;
	printk("%s:%d Configuring i2c_pull_up\n", __func__, __LINE__);
	...
	if (escore->i2c_pull_up) {
		...
		printk("%s:%d regulator_count_voltages %d",
				__func__, __LINE__,
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (np) {
		...
		printk("%s:%d i2c_pull_up %d\n",
			__func__, __LINE__, escore->i2c_pull_up);

		rc = escore_i2c_regulator_configure(escore);asdfdf
		if (rc) {
...
continue_init:
	...
	escore_device_startup_seq(escore->dev);
#ifdef CONFIG_SND_SOC_ES_ASYNC_FW_LOAD
