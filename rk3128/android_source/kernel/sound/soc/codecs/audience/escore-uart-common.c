...
#define UART_TTY_DEVICE_NODE "/dev/ttyS1"
...
int escore_uart_open(struct escore_priv *escore)
{
	...
	if (escore->uart_ready) {
		printk("%s() UART is already open, users count: %d\n",
	...
	do {
		if (attempt++ > 0)
			msleep(100);
		printk("%s(): probing for tty on %s (attempt %d)\n",
	...
	/* device node found */
	printk("%s(): successfully opened tty\n", __func__);
...
int escore_uart_close(struct escore_priv *escore)
{
	...
	if (atomic_dec_return(&escore->uart_users)) {
		printk("%s() UART is busy to close, user count: %d\n",
			 __func__, atomic_read(&escore->uart_users));
		return 0;
	}
