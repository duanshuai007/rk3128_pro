...
static int rfkill_rk_setup_power(struct rksdmmc_gpio *gpio, const char* prefix, const char* name){
	if (gpio_is_valid(gpio->io)) {
		int ret=0;
		sprintf(gpio->name, "%s_%s", prefix, name);
		ret = gpio_request(gpio->io, gpio->name);
		...
		ret = gpio_direction_output(gpio->io, 1);
		if (ret) {
			LOG("Failed to get %s gpio.\n", gpio->name);
			return -1;        
			}		
		msleep(2500);
		ret = gpio_direction_output(gpio->io, 0);
		if (ret) {
			LOG("Failed to get %s gpio.\n", gpio->name);
			return -1;
			}
...
static int rfkill_rk_setup_reset(struct rksdmmc_gpio *gpio, const char* prefix, const char* name){
	if (gpio_is_valid(gpio->io)) {
		int ret=0;
		sprintf(gpio->name, "%s_%s", prefix, name);
		ret = gpio_request(gpio->io, gpio->name);
...
static int rfkill_wlan_probe(struct platform_device *pdev)
{
    ...
    if (!pdata->mregulator.power_ctrl_by_pmu) {
     ...
     ret = rfkill_rk_setup_power(&pdata->power_n, wlan_name, "wlan_poweren");
        if (ret) goto fail_alloc;

		
		// Turn reset wifi power as default
    ...
    ret = rfkill_rk_setup_reset(&pdata->reset_n, wlan_name, "wlan_reset");
        if (ret) goto fail_alloc;
    }
