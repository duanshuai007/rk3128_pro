...
extern void vcc_sd_enable(int value);
#endif
...
static void dw_mci_work_routine_card(struct work_struct *work)
{
	...
	for (i = 0; i < host->num_slots; i++) {
		...
		*/
		...
		if(present ){
				//rt5036_contrl_lsw1(1);
				if(IS_ERR(host->vmmc))
				    host->vmmc = devm_regulator_get(mmc_dev(mmc), "vmmc");
				if (IS_ERR(host->vmmc)) {
    		        printk("%s: no vmmc regulator found\n", mmc_hostname(mmc));
    		        host->vmmc = NULL;
    	        }else{
    	        	printk("regulator found------------------------\n");
    	        	//regulator_enable(host->vmmc);
                       // regulator_set_voltage(ldo_9, 2800000, 2800000);
                       // regulator_enable(ldo_9);
		...
		vcc_sd_enable(1);
			...
			else
			{
				...
				if (IS_ERR(host->vmmc)) {
    	        	...
    	        	printk("regulator found regulator_disable\n");
#ifdef CONFIG_MFD_RK818
		vcc_sd_enable(0);
#endif
		//	regulator_disable(ldo_9);
                       // regulator_put(ldo_9);
