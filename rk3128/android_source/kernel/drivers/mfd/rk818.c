...
int vcc_codec_vcc_enable(int value)
{
	struct rk818 *rk818 = g_rk818;
	printk("vcc_codec_vcc_enable\n");
	if(value == 1)
		rk818_set_bits(rk818, RK818_LDO_EN_REG, 1 << 1, 1 << 1); //ldo2
	else
		rk818_set_bits(rk818, RK818_LDO_EN_REG, 1 << 1, 1 << 0); //ldo2

    return 0;
}
EXPORT_SYMBOL_GPL(vcc_codec_vcc_enable);

int vcccif28_enable(int value)
{
	struct rk818 *rk818 = g_rk818;
	printk("vcccif28_enable_enable\n");
	if(value == 1){
		rk818_set_bits(rk818, RK818_LDO_EN_REG, 1 << 3, 1 << 3); //ldo4
		rk818_set_bits(rk818, RK818_LDO_EN_REG, 1 << 7, 1 << 7); //ldo8
	}
	else{
		rk818_set_bits(rk818, RK818_LDO_EN_REG, 1 << 7, 1 << 0); //ldo8
		rk818_set_bits(rk818, RK818_LDO_EN_REG, 1 << 3, 1 << 0); //ldo4
	}

    return 0;
}
EXPORT_SYMBOL_GPL(vcccif28_enable);


int vcc_sd_enable(int value)
{
	struct rk818 *rk818 = g_rk818;
	if(value == 1)
		rk818_set_bits(rk818, RK818_DCDC_EN_REG, 1 << 5, 1 << 5); //ldo9
...
EXPORT_SYMBOL_GPL(vcc_sd_enable);

int vcc_lcd_enable(int value)
{
	struct rk818 *rk818 = g_rk818;

    printk("--->vcc_lcd_enable\r\n");
...
EXPORT_SYMBOL_GPL(vcc_lcd_enable);
...
static int rk818_ldo_disable(struct regulator_dev *dev)
{
	struct rk818 *rk818 = rdev_get_drvdata(dev);
	int ldo= rdev_get_id(dev) - RK818_LDO1;
	printk("rk818_ldo_disable ldo = %d \n",ldo);
	if(ldo == 1)
		return 0;
	if (ldo == 8)
		 rk818_set_bits(rk818, RK818_DCDC_EN_REG, 1 << 5, 1 << 0); //ldo9
...
//	ret = rk818_set_bits(rk818, 0xa1, (0xF<<0),(0x7));
    ret = rk818_set_bits(rk818, 0xa1, (0x4),(0x4));//rk818_set_bits(rk818, 0xa1, (0xF<<0),(0x7))
	rk818_set_bits(rk818, RK818_LDO_EN_REG, 1 << 1, 1 << 1); //ldo2
