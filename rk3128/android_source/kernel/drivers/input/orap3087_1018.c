	...
	{	
...
#endif		
		{
			...
			//	printk("prox  als_date2 = %d \n",prox);
				date +=  (prox << 8);
				input_report_abs(Orap_priv->input_dev, ABS_MISC, date);
...
////////////////////////////////////////////
	...
	Orap_priv->input_dev->name = "lightsensor";	
	input_set_capability(Orap_priv->input_dev, EV_ABS, ABS_MISC);	

	
	input_set_abs_params(Orap_priv->input_dev, ABS_MISC, 0, 4095, 0, 0);
	
	
	int ret = input_register_device(Orap_priv->input_dev);
	if (ret < 0) {	
...
static int orap3087_suspend(struct i2c_client *client){
	...
	hrtimer_cancel(&Orap_priv->timer);
	cancel_work_sync(&Orap_priv->ls_work);

	//printk("orap3087_suspend================================\n");
}


static int orap3087_resume(struct i2c_client *client){
...
static struct i2c_driver AP3087B_driver = {
	...
	.id_table = AP3087B_id,	
#if 1	
	.suspend  	= orap3087_suspend,	
    .resume  	= orap3087_resume,
#endif    
	/* match */
...
{	
	...
	int ret = i2c_add_driver(&AP3087B_driver);
