...
static ssize_t bat_info_store_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	u8 buf1;

	struct rk818_battery *di = to_bat_device_info(dev_get_drvdata(dev));

	buf1 = rk818_bat_read(di, RK818_MISC_MARK_REG);
	if (buf1 < 0) {
		printk("error reading TS_CTRL_REG");
		return buf1;
	}
...
static struct device_attribute rk818_bat_attr[] = {
	__ATTR(bat, 0664, bat_info_store_read, bat_info_store),
};
				     ...
				     enum charger_type charger_type)
{
	...
	case DC_TYPE_NONE_CHARGER:
		 ...
		 */
		 printk("DC_TYPE_NONE_CHARGER 1\n");
		buf = rk818_bat_read(di, RK818_VB_MON_REG);
		if ((buf & PLUG_IN_STS) == 0) {
			di->ac_in = OFFLINE;
			di->usb_in = OFFLINE;
			di->prop_val = POWER_SUPPLY_STATUS_DISCHARGING;
			printk("rk818_bat_read 1\n");
			rk818_bat_set_current(di, INPUT_CUR450MA);
		} else if (di->usb_in) {
			rk818_bat_set_current(di, INPUT_CUR450MA);
			di->prop_val = POWER_SUPPLY_STATUS_CHARGING;
			printk("rk818_bat_read 2\n");
		}else if (di->dc_in == OFFLINE) {
			//rk818_bat_set_current(di, INPUT_CUR450MA);
			di->prop_val = POWER_SUPPLY_STATUS_DISCHARGING;
			printk("rk818_bat_read 3\n");
		}
		power_supply_changed(&di->usb);
		power_supply_changed(&di->ac);
		break;
...
static irqreturn_t rk818_vbat_dc_det(int irq, void *bat)
{
	...
	queue_delayed_work(di->charger_wq,
			   &di->dc_delay_work, msecs_to_jiffies(350));
	rk_send_wakeup_key();
...
static enum charger_type rk818_bat_init_dc_det(struct rk818_battery *di)
{
	...
	if (gpio_is_valid(di->pdata->dc_det_pin)) {
		...
		ret = devm_request_threaded_irq(di->dev, dc_det_irq,NULL, rk818_vbat_dc_det,
				       irq_flags | IRQF_ONESHOT, "rk818_dc_det", di);
		if (ret != 0) {
...
static void rk818_bat_init_charger(struct rk818_battery *di)
{
	...
	rk818_bat_init_dc_det(di);
	dc_charger = rk818_bat_get_acusb_state(di);
