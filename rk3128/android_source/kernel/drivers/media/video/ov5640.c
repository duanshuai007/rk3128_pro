...
static struct rk_sensor_reg sensor_init_data[] ={
  ...
  {0x3031, 0x00},//inner regulator   //DOVDD 2.8V 0x08     1.8v: 0x00
  {0x302c, 0x42}, 
...
*/
	static int sensor_parameter_record(struct i2c_client *client)
	{
     ...
     sensor_write(client,0x3a00,0x38); //stop night mode   
     
     sensor_read(client,0x3500, &ret_h);
     sensor_read(client,0x3501, &ret_m);
     sensor_read(client,0x3502, &ret_l);
     
     ulCapture_Exposure = ((((ret_h&0x0f)<<8)+ret_m)<<4)+(ret_l>>4);
     
     ulCapture_Exposure = ulCapture_Exposure/3; 
