...
int
wl_ext_set_keep_alive(struct net_device *dev, char *command, int total_len)
{
	wl_mkeep_alive_pkt_t *mkeep_alive_pktp;
	int ret = -1;
	int	id, period, len_bytes=0, buf_len=0;
	char data[200]="\0";
	...
	if (ret)
		printf("%s: set mkeep_alive failed %d\n", __FUNCTION__, ret);
		...
		int *bytes_written)
{
	...
	if(strnicmp(command, CMD_GET_CHANNEL, strlen(CMD_GET_CHANNEL)) == 0) {
		...
		*bytes_written = wl_ext_set_keep_alive(net, command, total_len);
	}
	else if (strnicmp(command, CMD_GET_KEEP_ALIVE, strlen(CMD_GET_KEEP_ALIVE)) == 0) {
		*bytes_written = wl_ext_get_keep_alive(net, command, total_len);
	}
